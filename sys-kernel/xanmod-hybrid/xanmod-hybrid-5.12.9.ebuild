...
DESCRIPTION="Xanmod and Xanmod-CaCule patchset for main kernel tree"
...
src_prepare() {
	...
	if  use xanmod  ;   then
		eapply  "${FILESDIR}/patch-5.12.9-xanmod1"  ||  die
	fi

	if  use cacule  ;   then
		eapply  "${FILESDIR}/patch-5.12.9-xanmod1-cacule"   ||  die
	fi
